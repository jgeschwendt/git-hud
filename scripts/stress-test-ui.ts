...
async function main() {
	...
	try {
		...
		let branchesToCreate = [
			"ui-wt-1", "ui-wt-2", "ui-wt-3", "ui-wt-4", "ui-wt-5",
			"ui-wt-6", "ui-wt-7", "ui-wt-8", "ui-wt-9", "ui-wt-10",
		];
		...
		if (await existingRepo.isVisible({ timeout: 2000 }).catch(() => false)) {
			...
			if (existingRepoData?.worktrees?.length > 0) {
				...
				if (existingBranches.length > 0) {
					...
					log(`Will recreate ${branchesToCreate.length} existing worktrees: ${branchesToCreate.join(", ")}`);
				}
			}
			...
			const deleteBtn = repoRow.locator('button[title="Delete repository"]').first();
		...
		const urlInput = page.locator('input[placeholder="git@github.com:user/repo.git"]');
		...
		const worktreeInput = repoTable.locator('input[placeholder="feature/new-branch"]').first();
		...
		// Take screenshot
		await page.screenshot({ path: "stress-test-ui-result.png", fullPage: true });
