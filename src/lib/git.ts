 ...
 * Sets up tracking to remote branch when possible (fork-friendly)
 */
export async function createWorktree(
  ...
  try {
    ...
    if (newBranch) {
      // Create new branch tracking remote (if remote branch exists)
      // git worktree add --track -b <branch> <path> <remote>/<branch>
      // Falls back to just -b if remote branch doesn't exist
      const remoteRef = `${remote}/${branch}`
      const refCheck = await execGit(['rev-parse', '--verify', `refs/remotes/${remoteRef}`], repoPath)

      if (refCheck.success) {
        // Remote branch exists - track it
        args = ['worktree', 'add', '--track', '-b', branch, worktreePath, remoteRef]
      } else {
        // No remote branch - create local only (will push later)
        args = ['worktree', 'add', '-b', branch, worktreePath]
      }
    } else {
      // Checkout existing branch - git handles tracking automatically
    ...
    // For existing branches without tracking, set it up
    if (!newBranch) {
      const trackingCheck = await execGit(
        ['config', '--get', `branch.${branch}.remote`],
        worktreePath
      )
      if (!trackingCheck.success) {
        // No tracking - check if remote branch exists and set up tracking
        const remoteRef = `${remote}/${branch}`
        const refCheck = await execGit(['rev-parse', '--verify', `refs/remotes/${remoteRef}`], repoPath)
        if (refCheck.success) {
