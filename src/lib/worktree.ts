...
export async function createWorktree(
	...
	try {
		...
		let result = await gitCreateWorktree(repo.local_path, worktreePath, branch, false);
		if (!result.success && result.error?.includes("invalid reference")) {
			setProgress(worktreePath, "Creating new branch...");
			result = await gitCreateWorktree(repo.local_path, worktreePath, branch, true);
		}
		if (!result.success) {
